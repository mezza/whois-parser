...
describe "whois.comlaude.com", :aggregate_failures do
  ...
  it "matches status_registered.expected" do
    expect(subject.domain).to eq("comlaude.com")
    expect(subject.domain_id).to eq("140988980_DOMAIN_COM-VRSN")
    expect(subject.status).to eq(:registered)
    expect(subject.available?).to eq(false)
    expect(subject.registered?).to eq(true)
    expect(subject.created_on).to be_a(Time)
    expect(subject.created_on).to eq(DateTime.parse("2005-01-30 11:52:54 UTC"))
    expect(subject.updated_on).to be_a(Time)
    expect(subject.updated_on).to eq(DateTime.parse("2025-10-08 12:44:46 UTC"))
    expect(subject.expires_on).to be_a(Time)
    expect(subject.expires_on).to eq(DateTime.parse("2027-01-30 00:00:00 UTC"))
    expect(subject.registrar).to be_a(Whois::Parser::Registrar)
    expect(subject.registrar.id).to eq("470")
    expect(subject.registrar.name).to eq("NOM-IQ Ltd dba Com Laude")
    expect(subject.registrar.url).to eq("https://www.comlaude.com")
    ...
    expect(subject.registrant_contacts[0].address).to eq("28-30 Little Russell Street")
    expect(subject.registrant_contacts[0].city).to eq("London")
    expect(subject.registrant_contacts[0].zip).to eq("WC1A 2HN")
    expect(subject.registrant_contacts[0].state).to eq("")
    expect(subject.registrant_contacts[0].country_code).to eq("GB")
    expect(subject.registrant_contacts[0].phone).to eq("+44.2074218250")
    ...
    expect(subject.admin_contacts[0].email).to eq("comlaude.com-Admin@anonymised.email")
    ...
    expect(subject.technical_contacts[0].email).to eq("comlaude.com-Tech@anonymised.email")
